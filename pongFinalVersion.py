...
# The dot passes through only the back side of the paddle.
# The score appears and is contiually updated.
...
class Game:
   ...
   def play(self):
      ...
      while self.close_clicked == False:  # until player clicks close box
         ...
         self.game_Clock.tick(self.FPS) # runs at most with FPS Frames Per Second 
   ...
   def draw(self):
      ...
      # Makes the updated surface appear on the display
   ...
   def collison(self):
      # Checks if dot collided with paddle
      self.center = self.dot.re_center()
      self.velocity = self.dot.re_velocity()
      # Left paddle collison detection
      ...
      # Right paddle collison detection
...
class Dot:
   ...
   def set_velocity(self):
      # Negates the dot velocity
      self.velocity[0] = -self.velocity[0]
            
   def r_edge(self):
      # Checking dot's right edge bounce
      if self.center[0] + self.radius > 500:
         return True
      
   def l_edge(self):
      # Checking dot's left edge bounce
